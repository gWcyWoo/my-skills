...
**Do NOT inspect test-file bodies** (`*.test.*`, `*.spec.*`, `__tests__/**`, `*.e2e.*`, and similar test-runner config or fixture files) during requirements analysis. Test files are out of scope for analysis, even if `my-explore` or `my-explore-0` later identifies a related test file that should be listed in Affected Files.
