---
name: understand
description: Use when requirement analysis with optional HLD and file persistence is needed before implementation, TDD, or auto-tdd, with the user kept in the loop in the main Codex session.
---
...
- Reading or exploring the codebase
- Explaining existing code
- Configuration-only changes (no code logic), unless the user explicitly requests analysis
- The user explicitly instructs to skip analysis

## Code Navigation Rule

**The `my-explore` family is the ONLY permitted code navigation methodology.** In the main session, use `my-explore`. If this workflow is ever resumed inside a subagent that cannot dispatch again, use `my-explore-0`. Brainstorming explores requirements and ideas; `my-explore` and `my-explore-0` explore code. Never use any other approach for code navigation, regardless of what other loaded skills suggest.

## Process

### Step 1: Explore & Analyze

1. Choose the exploration variant by execution context:
   - Main session -> invoke `my-explore`
   - Already inside a subagent that cannot recursively dispatch -> invoke `my-explore-0`
...
2. Follow `/Users/Woo/.agents/skills/understand/analysis-instructions.md` for the analysis process (classification, analysis, AC extraction, complexity gate). **The user's conversation message is the requirement input. Ignore any `requirement.md` file references. Present the analysis in conversation. Do NOT write files yet.**
   ...
   **Brainstorming trigger**: During analysis, if you encounter any of the following, invoke the `brainstorming` skill. Do NOT ask the user first:
   ...
   **Code exploration during brainstorming MUST still use the `my-explore` family only; no other navigation approach.** After brainstorming completes, continue the analysis with the refined understanding.
...
4. **STOP and present** the complete understanding to the user:
   ...
   - Wait for the user to confirm, correct, or request changes
...
> Select next step:
> 1. **code-0** - Implement directly in Codex.
> 2. **tdd** - Lightweight TDD: user stays in the loop for test design and implementation.
> 3. **auto-tdd** - Automated: Codex runs the persisted parallel workflow. (Requires persisted files)

- `code-0` (or equivalent user intent to proceed with direct implementation) -> invoke the `code-0` skill. Use conversation context if files were not persisted.
- `tdd` (or equivalent user intent to start with tests) -> invoke the `tdd` skill. Lightweight flow with user in the loop.
- `auto-tdd` (or equivalent user intent to run the automated parallel workflow) -> invoke the `auto-tdd` skill, passing the procedure directory path. **Requires files to be persisted in Step 2.** If not persisted, ask the user to persist first.
