...
- Relative descriptions: "behavior remains unchanged", "same as before", "unaffected" — these are not testable. Rewrite as positive statements of expected behavior.
...
**Do NOT write self-check tables to any file.** The author only ensures ACs are clean before writing output.
...
- **No-logic** → Invoke the `self-check` skill using the Skill tool, passing:
  - `rules_path`: `/Users/Woo/.agents/skills/understand/self-check.rules.md`
...
Read `/Users/Woo/.agents/skills/hld/SKILL.md` and follow its process exactly to produce `{procedure_dir}/hld.md`. Use the analysis context you already built.
