...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=["assembly", "board", "fetch", "contract"], default="assembly",
...
1. Write the row's interaction text to {spec / "interaction.txt"}, then: parse_interactions.py -> check_interaction_completeness.py (ignored sentences carrying trigger+effect signals must become rules or be registered as acknowledgedNonRules — the gate must pass).
...
3. resolve_interaction_anchors.py --index {Path(args.project_root).resolve() / ".iff" / "board_index.json"} -> YOU confirm ambiguous anchors within the listed candidates only, mark zero-hit references pending_route (cross-row targets; tests assert navigation intent against mocks) -> rerun with --check, must pass.
4. normalize_api_contract.py --oas {spec / "oas.json"} (prefetched by main in 2.5; if missing, fetch via the Apifox MCP tool FIRST) -> make_interaction_tests_plan.py.
Reading discipline: read case memory (injected in project CLAUDE.md) BEFORE anchor/edge judgments and record hit CASE-ids; interaction_contract.json/api_contract.json/small files may be read whole; NEVER read oas.json, scene.json, render_plan.json or raw.json in full.
Allowed writes: ONLY feature-level contract artifacts under {spec} (interaction.txt, interaction_contract.json, interaction_completeness_report.json, state_machine.json, interaction_anchors.json, api_contract.json, interaction_test_plan.json). FORBIDDEN: any lib/ or test/ file, any board dir's canvas/expected/slots, pubspec/routes/DI, ANY `flutter` command.
...
- ASSEMBLY role (board-level fan-out): board workers already produced each state's canvas/expected/slots/merged_expected/implementation_map — CONSUME them, never regenerate or edit board canvases. You own: selector/page/colors/one shared fixture/slot mapper, CONSUMING the contract worker's step-6 outputs (interaction_contract/state_machine/anchors/test plan are already produced and --check-passed in parallel with board workers — verify they exist and their check gates pass, NEVER regenerate or re-derive them; missing/failed contract artifacts = stop and return failure), prefill plan + modelFields, TDD (ONE red run, ONE green run), data integration, the device window (9-11, device_lock held), audits (12) with trace tests for every state generated by you and executed inside the ONE audit-time `flutter test test/<feature>` (feature-scoped — the whole-repo regression run belongs to the main session's serial fan-in, never to you), and pubspec/fonts/asset registration for this feature's collected assets.
...
- Only device I/O (install/launch/screenshot) is mutex-guarded — builds are NOT: pre-build outside the lock (flutter build apk --debug / flutter build ios --simulator --debug), run device_lock.py acquire right before capture and release IMMEDIATELY after the capture on EVERY exit path (success, failure, error). Repair edits and rebuilds happen outside the lock; re-acquire only for the single re-capture (two short I/O windows, never one long hold). If the main session supplies a device pool, acquire with --pool returns your device id as JSON on stdout — pass exactly that device to capture_runtime_screenshot.py and release with --device. Everything else runs in parallel with sibling workers.
