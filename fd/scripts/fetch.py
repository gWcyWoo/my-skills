...
Cookie resolution order:
    1. --cookie <value>
    2. $LANHU_COOKIE
    3. ~/.claude/mcp/lanhu-mcp/.env  (LANHU_COOKIE="...")
    4. ~/.agents/mcp/lanhu-mcp/.env  (Codex-side fallback)
"""
...
def main():
    ...
    if not cookie:
        print(
            ...
            "into ~/.claude/mcp/lanhu-mcp/.env "
            "(Codex-side ~/.agents/mcp/lanhu-mcp/.env is also accepted)",
            file=sys.stderr,
        )
        sys.exit(1)
