...
- Never call `mcp__language_server__get_symbol_definitions` more than ONCE per exploration turn. Multi-symbol exploration goes through `probe.search_code` (concept phrase) + a single batched `extract_code(files=[...])`, not through N sequential `get_symbol_definitions` calls. A 4-symbol exploration must resolve in ≤ 2 turns, not 4.
...
- **Whitelist for file boundary** — `wc -l <path>` (via bash / shell) is the ONLY allowed way to confirm a file's total line count when the extract result did not expose it. Never use `start_server` + `get_symbols` as a substitute; it costs a 120s timeout and returns the same information.
