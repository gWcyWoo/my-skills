...
If any of these files are missing, stop and display the error. Do NOT invoke `u-0` or create these files here - they must be produced before invoking this skill.
...
The main session acts as a **dispatcher** - it runs `u-0` for each requirement, then delegates the auto-tdd pipeline to a child agent.
