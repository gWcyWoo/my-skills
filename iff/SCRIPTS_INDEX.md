...
| 脚本 | 消费 | 产出 |
|---|---|---|
| `make_render_plan` | scene.json, assets, layout_contract | **render_plan.json**(每节点渲染方式;`--root-node` 时含 `rootNode` 组件坐标空间) |
...
| `generate_canvas` | **render_plan, tokens** | **canvas.dart + expected.json + slots.json + optional colors**(无颜色 token 时不生成 colors import/file;带 key 数据驱动画布,模型写0像素) |
| `copy_assets` | assets_manifest, target | assets/images/* |
| `update_pubspec_assets` | pubspec, asset | pubspec.yaml 注册 |
| `sync_project_rules` | implementation_rules, project-root | 注入工程 AGENTS.md |
...
| 脚本 | 消费 | 产出 |
|---|---|---|
| `oas_ref_resource_cache missing/merge` | OAS root + 当前资源缓存 / MCP 增量响应 + required 路径 | 稳定排序 missing 路径 / 原子合并缓存 |
| `normalize_api_contract` | OAS root + 闭包完整的 ref-resource cache(Apifox) | **api_contract.json** |
...
## 编排 / 辅助(不在链上)
- `verify_pipeline_scripts`(预检:58 脚本齐全)· `make_worker_prompt`(命令式 worker todo)· `generate_shared_component_test`(从 pubspec package name 生成共享 widget 测试)。
