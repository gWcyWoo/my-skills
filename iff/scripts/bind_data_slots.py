...
def main() -> int:
    ...
    for comp in manifest.get("components", []):
        for slot in comp.get("dynamicSlots", []):
            ...
            entry = {
                ...
                "designText": text,
                "binding": match,
                "contentSource": {"kind": "design", "evidence": "component_manifest.designText"},
    ...
    out = {
        ...
        "note": "Model must confirm every binding against the interaction spec and set confirmedByModel=true before the API gate. When a value is returned by an API, set contentSource.kind=api and provide exact endpoint/response-field or repository-field evidence; its runtime value need not equal designText.",
    }
