...
固定实现方式:根节点固定 artboard 宽高;外层用 `FittedBox` 或固定 artboard canvas 做响应式适配,避免 `Transform.scale` 造成 widget test 命中异常;可见层必须是单一坐标画布,每个 visible render node 按 `render_plan` bbox 生成 `Positioned` + 固定 `SizedBox`;背景、输入框、按钮等节点必须按 bbox 填满,不得依赖子组件 intrinsic size;文字使用 `fontSize/weight/lineHeight/color`;shape 使用 tokens 中的颜色、圆角、边框、阴影;装饰、图标、图片使用真实独立 asset;语义组件、业务按钮和命中区域只能透明覆盖在坐标画布上,不得参与可见像素布局。
...
python3 ~/.agents/skills/iff/scripts/make_repair_plan.py --diff spec_dir/diff_report.json --layout spec_dir/layout_contract.json --render-plan spec_dir/render_plan.json --implementation-map spec_dir/implementation_map.json --actual-trace spec_dir/actual_layout_trace.json --out spec_dir/repair_plan.json
```
...
python3 ~/.agents/skills/iff/scripts/make_repair_plan.py --diff spec_dir/diff_report.json --layout spec_dir/layout_contract.json --render-plan spec_dir/render_plan.json --implementation-map spec_dir/implementation_map.json --actual-trace spec_dir/actual_layout_trace.json --out spec_dir/repair_plan.json
```
硬门: CSV 行仍是 `doing`;worker 已加载当前 iFF 规则且 compliance 校验通过;reference 是完整 artboard;actual 是真实模拟器截图;fixture 同源;交互 case 覆盖和 red/green 证据通过;多状态数量一致;`render_plan` 未使用整图冒充;页面结构由真实组件、文本、按钮、卡片、输入框、状态区域组成;diff 达到阈值;`repair_plan.json` 存在且 `summary.p0Count=0`;test/analyze 通过。全部满足后才写 `status=done`、`actual_screenshot`、`visual_manifest`、`visual_report`。
</pipeline>
...
- 每行 `done` 前必须强校验:CSV 原 status 还不是 `done`;`worker_compliance.json` 证明 worker 加载了当前 iFF 规则;`reference.png` 是完整 artboard;最终 `actual.png` 来自真实运行截图;manifest provenance 有效;测试 fixture 与 app runtime fixture 同源;`interaction_test_plan.json` 的每个 case id 都映射到测试且有 red/green evidence;多状态设计稿的运行截图呈现同一组状态;`render_plan.json` 通过整图冒充审计;页面结构不是整图铺底;单次 repair 后的最终 `diff_report.json` 达到 99% 阈值;`repair_plan.json` 存在且没有 P0 action;`flutter test` 通过;`flutter analyze` 通过。
...
- `flutter test` 与 `flutter analyze` 无 error;每个已处理行的 worker 都有当前 iFF 规则 compliance;每个 feature 落地且符合 `reference.png` + `scene.json` + `groups.json` + `tokens.json` + `assets_manifest.json` + `layout_contract.json` + `render_plan.json` + `interaction_contract.json` + `interaction_test_plan.json` + `repair_plan.json`;页面由真实 Flutter 组件逐层渲染,不是整图铺底;交互 case 覆盖和 red/green evidence 通过;测试 fixture 与 app runtime fixture 同源;多状态设计稿在真实 app shell 中呈现同一组状态;最终真实设备 `actual.png`、`visual_manifest.json`、`diff_report.json` 达到 99% 阈值且 `repair_plan.json` 无 P0 action。
</success_criteria>
