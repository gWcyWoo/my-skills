...
- **With HLD**: Use HLD-defined function signatures and interfaces as the source of truth. Do not invent APIs.
- **Without HLD**: Read source files from `understand` → Affected Files. Extract actual function signatures, return types, parameters. These become the API contracts.
...
Unit tests are **auto-generated by AI** from code analysis. AI proactively derives test cases from function signatures, branches, and edge cases. The user does not need to enumerate every test — AI discovers them.

- **AC Coverage**: One or more tests per in-scope Acceptance Criterion.
- **Path Coverage**: Every success path and error handling path. AI analyzes code branches to ensure full path coverage.
