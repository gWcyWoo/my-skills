...
def writeback_args(f, *extra):
    return (WRITEBACK, "--values", f.raw, "--link", f.sheet,
            "--spreadsheet-id", "doc-123", "--sheet", "Sheet1", *extra)
...
class TestWriteback(unittest.TestCase):
    ...
    def test_ranges_follow_header_names_not_positions(self):
        ...
        res = sh(*writeback_args(f, "--role", "frontend", "--row-ids", "r2"))
        ...
        self.assertEqual(res["mcp_args"], {
            "spreadsheet_id": "doc-123", "sheet": "Sheet1",
            "ranges": {"K2": [["ready"]], "L2": [[""]], "M2": [[""]],
                       "Q2": [[""]], "R2": [[""]], "S2": [[""]]},
        })
    ...
    def test_unknown_row_halts(self):
        f = Fixture([HEADER, row(title="登录", fstatus="ready")])
        res = sh(*writeback_args(f, "--role", "frontend", "--row-ids", "r99"), expect=1)
    ...
    def test_missing_role_column_halts(self):
        ...
        f.raw.write_text(json.dumps([header, [""] * len(header)], ensure_ascii=False),
                         encoding="utf-8")
        res = sh(*writeback_args(f, "--role", "frontend", "--row-ids", "r2"), expect=1)
        self.assertEqual(res["errors"][0]["code"], "missing_column")

    def test_codex_error_envelope_preserves_doc_not_found(self):
        f = Fixture([HEADER, row(title="登录", fstatus="ready")])
        f.raw.write_text(json.dumps({"content": [{"type": "text", "text":
                         "Error executing tool get_sheet_data: HttpError 404"}],
                         "isError": True}), encoding="utf-8")
        res = sh(*writeback_args(f, "--role", "frontend", "--row-ids", "r2"), expect=1)
        self.assertEqual(res["errors"][0]["code"], "doc_not_found")
