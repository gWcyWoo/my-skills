...
真正的 `batch_update_cells` MCP 调用是模型 I/O,归模型。

用法:
  sheet_writeback.py --values <raw.json> --link <sheet.json> --sheet <名>
                     --role <role> --row-ids r2,r8

输出 updates[*] 的 `range`/`value` 即 batch_update_cells 的入参。
"""
...
from sheet_normalize import ROLE_FIELDS, build_index, emit, extract_values  # noqa: E402
    ...
    for rid in want:
        ...
        for field in ROLE_FIELDS:
            updates.append({
                "row_id": rid,
                "field": field,
                "range": f"{args.sheet}!{a1_col(idx[f'{args.role}.{field}'])}{row_index}",
