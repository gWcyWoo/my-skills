...
def resolve_cookie(cli_cookie):
    ...
    if env_v:
        return env_v
    for dotenv in (os.path.expanduser("~/.codex/mcp/lanhu-mcp/.env"),
                   os.path.expanduser("~/.claude/mcp/lanhu-mcp/.env")):
        cookie = _cookie_from_dotenv(dotenv)
        if cookie:
            return cookie
    return None
...
def main():
    ...
    if not cookie:
        return emit_error("no_cookie",
                          ...
                          "写 LANHU_COOKIE=...(~/.claude/mcp/lanhu-mcp/.env 亦可)")
