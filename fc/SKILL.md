...
<context>
Rule files:
- `~/.claude/skills/fc/development_rules.md`
...
```bash
python3 ~/.claude/skills/fc/scripts/inspect_layers.py --raw-json <path> [--ids ID,ID] [--type textLayer|shapeLayer|groupLayer|artboard] [--name-contains SUBSTR]
```

Static check:

```bash
python3 ~/.claude/skills/fc/scripts/check_static.py --raw-json <spec_dir>/raw.json --code-dir {{OUTPUT_DIR}} [--font-scale {{PT_SCALE}}]
```
...
6. Design strictly in this order:
   ...
   - Read `~/.claude/skills/fc/development_rules.md` before `flutter-widget.md`.
