...
3. **Explore code via `my-explore-0` ONLY.** When you need to read or query source code, invoke the `my-explore-0` skill. Never read source files directly in the main session.
4. **Implement the change.** Apply the rules `comply` returned. Touch only the resolved files. Use `apply_patch` for manual edits, including genuinely new files when needed. If the caller passed red test files, write the minimum implementation that turns them green. NEVER modify the tests.
