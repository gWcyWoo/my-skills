...
4. **Gate check.** Before proceeding, ask yourself: *"Have I run comply on the diff yet?"* If no → go to step 5. If yes → go to step 7.
5. **Load coding standards from diff.** Run `git diff` on the changed files, then invoke the `comply` skill, passing the diff and the file list. This gives `comply` concrete code to match rules against.
6. **Self-check against comply rules.** Re-read the rules `comply` returned. Fix any violations.
7. **Pause for review.** Ask: *"Comply review done. Would you like to review before running lint and tests?"*
...
P0 — Step 4 gate check is mandatory. If you haven't run comply, you CANNOT reach step 7.
P0 — ALWAYS pause at step 7 after comply + self-check. Even if the change is one line.
