...
3. **Implement the change.** Touch only the resolved files. Use `StrReplaceFile` for existing files; use `WriteFile` to create files only when they do not yet exist. If the caller passed red test files, write the minimum implementation that turns them green. NEVER modify the tests.
...
P1 — Use `StrReplaceFile` for existing files; use `WriteFile` to create genuinely new files. Never overwrite an existing file wholesale.
