...
2. **Explore code via the `my-explore` subagent ONLY.** When you need to read or query source code, dispatch the `my-explore` subagent: `Agent(subagent_type="my-explore", prompt=<request per ~/.claude/agents/my-explore/PROTOCOL.md>)`. NEVER use `Read` or `Grep` on source files directly.
...
<final_reminders>
P0 — NEVER use `Read` or `Grep` on source files. ALL code exploration goes through the `my-explore` subagent. No exceptions.
