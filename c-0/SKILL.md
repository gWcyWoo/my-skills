---
name: c-0
description: Use as the implementation step of any workflow — loads project coding standards via the comply subagent, implements the change in the MAIN session, and gates on user review before lint/test. The `-0` suffix denotes main-session execution.
---
...
2. **Explore code directly in this session.** Follow the loaded tool-selection rules in `~/.claude/CLAUDE.md` `<tool-usage>` and `~/.claude/skills/shared/tools-ins.md`: prefer `probe.extract_code(files=["path#Symbol", ...])` with anchors over whole-file `Read`; use `LSP.findReferences` / `LSP.documentSymbol` for callers and file structure. Do NOT `Read` whole source files when an anchored extract will do.
...
<final_reminders>
P0 — ALL code exploration follows the session's loaded tool-selection rules (`~/.claude/CLAUDE.md` `<tool-usage>` + `~/.claude/skills/shared/tools-ins.md`). Prefer `probe.extract_code` with `#Symbol` anchors; never `Read` whole source files, never `Bash grep -r` on source paths.
...
P0 — If the caller passed red test files, the goal of step 3 is to make them green by writing implementation. If any red test passes *before* you write implementation, the test is wrong — report it to the user and STOP. Do NOT weaken the test, skip it, or modify it to pass. Faking green is a TDD red line, forbidden by global CLAUDE.md.
