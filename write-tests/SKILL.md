...
- If yes → user specifies what to test. **STOP and confirm** the user's test cases. Do NOT proceed to step 2 until user confirms the final list.

## 2. Load rules & write tests

Once all test types and scope are confirmed, use the `my-subagent` skill to dispatch a child agent that loads the relevant test rules. Do NOT call `spawn_agent` directly.
