---
name: paac
description: Implement, verify, or reuse one ThinkPHP 6 endpoint from the refreshed Apifox contract.
---
...
Scope: process exactly one Apifox interface unless the user names multiple interfaces.
...
3. Interpret the contract.
   ...
   - Use framework-provided context only when route/controller evidence shows the same endpoint category uses it. Do not expose framework context as API input.
...
4. Inspect existing code.
   - Follow the active `AGENTS.md` source-exploration rules for every target-repository evidence read.
   - Do not inspect target-repository files for PAAC evidence outside a pass with `current_evidence_category` set.
   - Required repository evidence is mode-dependent:
     - Controller-only mock mode: routes, target controller/action, contract input validation, response helper, auth/header middleware or config conventions, integration-test convention, test config, and test command. Do not inspect logic, service, model, or migration files unless the existing target action calls them or route/controller evidence conflicts.
     - Non-empty `description` mode: routes, target controller/action, validation, middleware/auth/config, response helpers, description-touched logic/service/model/migration files, test convention, test config, and test command.
   ...
   - If a route file is known but the exact route line is unknown, extract route windows anchored by the known route prefix, controller name, or action name. Do not re-search with different keywords.
   - A new exploration pass does not reset `search_used` for the same endpoint and same evidence category.
   - A later search is allowed only when `known_candidate_files[current_evidence_category]` is empty. The visible hypothesis plan must name that evidence category.
   ...
   - If additional target-repository evidence is required, start a new pass with `current_evidence_category` set and carry forward the ledger. Do not reset search permission for the same endpoint or evidence category. When candidate files exist for the category, use `extract_code` before any new search.
   - If exploration cannot obtain required evidence, stop and report the missing evidence. Do not fall back to direct shell reads.
...
5. Decide implementation action.
   ...
   - Edit only files identified through the PAAC ledger, or files whose paths are derived directly from the Apifox endpoint and existing project conventions returned by ledger evidence.
   - If patch context is insufficient, run another pass with `current_evidence_category` set. Do not read the target file directly to obtain patch context.
...
6. Test.
   - If Step 5 selects reuse with no behavior change and the only code change is a method-header comment update, skip TDD and report the reuse.
   - Load [tdd-gates.md](references/tdd-gates.md).
   - In controller-only mock mode, write integration contract tests for the confirmed Apifox success response and each confirmed Apifox error response. Do not write E2E tests, unit tests, or logic/service/model tests.
   - Follow the active `AGENTS.md` source-exploration rules for test style, fixture, bootstrap, PHPUnit/Pest configuration, helper, middleware, and command-discovery evidence.
   - Present the test plan and stop for user confirmation.
   - Write only confirmed tests.
   - Run the configured command that accepts the confirmed test file path(s). If no such command exists, stop and report the missing command.
   - Implement only behavior required by the confirmed tests and Apifox contract.
   - Run the red tests again, tests for files edited in Step 5, and the configured suite for the edited endpoint category. If no category suite exists, report that absence.
...
- Do not read target-repository evidence outside a pass with `current_evidence_category` set.
