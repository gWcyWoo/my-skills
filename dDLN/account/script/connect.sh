...
PROFILE="${1:?用法: connect.sh <profile> <host> <user> <port>}"
HOST="${2:?缺少 host}"
USER_="${3:?缺少 user}"
PORT="${4:-22}"
...
printf '正在连接 root@%s:%s —— 等 ssh 弹出 "password:" 提示再输入(输入不回显)...\n' "$HOST" "$PORT"
if ssh -M -S "$DIR/ctrl.sock" -o ControlPersist=1h -o ConnectTimeout=10 -fN -p "$PORT" "root@$HOST"; then
