...
If the same issue fails **3 consecutive times**, trigger **Stuck Escalation** (below).
...
- If no -> skip to 4d.
- If yes -> run E2E tests. If tests fail, apply the same rules as 4b.
...
2. **Research** -> search externally, for example GitHub issues, Stack Overflow, and technical blogs, for the specific error or pattern.
...
If the new approach also fails after 3 attempts, **STOP and present the situation to the user** with all findings and attempted approaches.
