...
Dispatch a subagent to extract only the relevant coding standards:

> 1. Read the project's `package.json` to identify frameworks and libraries in use.
...
>    - `next` in dependencies + backend/database files → `frontend/nextjs-fullstack.md`
>    - `express` in dependencies → `backend/express.md`
>    - `mongoose` or `mongodb` in dependencies → `backend/mongodb.md`
>    - Backend domain/application layer files → `backend/ddd.md`
