...
Wrap `spawn_agent` with a parent-owned polling workflow so the parent session can observe completion and resumability even when agent messages are not surfaced in real time.
...
1. Call `wait_agent` on the same `agent_id` with a 120-second timeout
...
2. send the user's answer back with `send_input`
...
- Calling `wait_agent` without a timeout and blocking the session indefinitely.
...
```text
spawn isolated agent with injected status instructions
every 20 seconds:
every 120 seconds:
  wait up to 120 seconds for terminal response
