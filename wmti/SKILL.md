...
Grounding: per global CLAUDE.md `<evidence>`, every claim rests on code re-read now. Locate before reading (grep/rg or LSP → exact file:line), read only the span that matters, then present.
</context>
...
P0 — Every claim cites a `file:line` re-read in this pass (global CLAUDE.md `<evidence>`). No claim without a fresh read.
